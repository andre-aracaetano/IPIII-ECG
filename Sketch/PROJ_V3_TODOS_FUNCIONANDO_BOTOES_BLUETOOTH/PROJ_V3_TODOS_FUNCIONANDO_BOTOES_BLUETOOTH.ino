...
void loop() {
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    if (!buttonPressed) {
      ...
      if (recording) {
        stopRecording();
        displayHeartWithText("PARADO", true);  // Coração estático no estado PARADO
        recording = false;
        estadoAtual = PARADO;  // Atualiza estado
        delay(3000);
      } else {
        startNewFile();
        displayHeartWithText("LENDO", true);  // Inicia o piscar do coração no estado LENDO
        recording = true;
        estadoAtual = LENDO;  // Muda o estado atual para LENDO
      }
    }
  } else {
    buttonPressed = false;
  ...
  if (digitalRead(BUTTON_PIN_BLUETOOTH) == LOW) {
    if (!buttonBluetoothPressed) {
      ...
      if (!bluetoothAtivo) {
        ...
        SerialBT.end();
        displayHeartWithText("PRONTO", true);  // Exibe o coração no estado PRONTO, sem piscar
  ...
  // Controle do piscar do coração apenas no estado LENDO
  if (estadoAtual == LENDO) {
    unsigned long currentMillis = millis();
    if (currentMillis - previousMillis >= heartInterval) {
      previousMillis = currentMillis;
      heartVisible = !heartVisible;  // Alterna a visibilidade do coração
      displayHeartWithText("LENDO", heartVisible);  // Coração pisca apenas no estado LENDO
    }
  } else if (estadoAtual == PARADO || estadoAtual == INICIANDO) {
    displayHeartWithText("PRONTO", true);  // Coração estático no estado PRONTO ou PARADO
  }
